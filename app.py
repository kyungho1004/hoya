...
def main():
    ...
    import json, os
    # 세션별 카운트
    if "session_views" not in st.session_state:
        st.session_state.session_views = 0
    st.session_state.session_views += 1
    # 앱 전체 누적 (파일 기반)
    ...
    try:
        if os.path.exists(VIEWS_FILE):
            with open(VIEWS_FILE, "r", encoding="utf-8") as f:
                _dat = json.load(f)
            total_views = int(_dat.get("count", 0))
        else:
            total_views = 0
    except Exception:
        total_views = 0
    ...
    try:
        with open(VIEWS_FILE, "w", encoding="utf-8") as f:
            json.dump({"count": total_views}, f, ensure_ascii=False)
    except Exception:
        pass
    st.sidebar.info(f"👀 조회수 — 세션: {st.session_state.session_views} · 전체: {total_views}")
