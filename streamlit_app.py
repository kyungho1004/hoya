...
st.caption("✅ 기본은 모바일 최적화 세로 입력. PC에서는 아래 '표 모드' 옵션으로 가로형 입력을 사용할 수 있어요. (모바일에선 자동으로 세로처럼 쌓입니다)")
...
# 입력 모드 전환 (표 모드: PC 친화 / 모바일에선 자동 세로 쌓임)
...
# Render base panel according to mode
if table_mode:
    render_inputs_table()
else:
...
if extra_panel:
    ...
    for item in extra_panel:
        if item["type"] == "num":
            vals[item["key"]] = st.number_input(item["label"], step=item.get("step", 0.1), key=f"ex_{item['key']}")
        elif item.get("type") == "select":
            opts = item.get("options", ["Unknown","Negative","Positive"])
            vals[item["key"]] = st.selectbox(item["label"], opts, key=f"ex_{item['key']}")
        elif item.get("type") == "text":
            vals[item["key"]] = st.text_input(item["label"], key=f"ex_{item['key']}")
        else:
            pass
...
st.caption("📱 모바일/PC 모두 줄꼬임 방지: 기본 세로 입력, 표 모드는 넓은 화면에서만 가로로 보이고 좁은 화면에서는 자동으로 세로로 쌓입니다. CRP는 0.01 단위로 입력됩니다.")
