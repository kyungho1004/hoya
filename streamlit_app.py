...
if run:
    ...
    if mode == "일반/암":
        lines = __import__('utils.interpret', fromlist=['interpret_labs']).interpret_labs(vals, extras)
        ...
        if nickname and "records" in st.session_state and st.session_state.records.get(nickname):
            ...
            cmp_lines = __import__('utils.interpret', fromlist=['compare_with_previous']).compare_with_previous(nickname, {k: vals.get(k) for k in ORDER if __import__('utils.inputs', fromlist=['entered']).entered(vals.get(k))})
            if cmp_lines:
                for l in cmp_lines: st.write(l)
            else:
                st.info("비교할 이전 기록이 없거나 값이 부족합니다.")

        shown = [ (k, v) for k, v in (locals().get('extra_vals') or {}).items() if __import__('utils.inputs', fromlist=['entered']).entered(v) ]
        if shown:
            st.markdown("### 🧬 암별 디테일 수치")
            for k, v in shown:
                st.write(f"- {k}: {v}")

        fs = __import__('utils.interpret', fromlist=['food_suggestions']).food_suggestions(vals, anc_place)
        if fs:
    ...
    if meds:
        ...
        for line in __import__('utils.interpret', fromlist=['summarize_meds']).summarize_meds(meds): st.write(line)

    if extras.get("abx"):
        abx_lines = __import__('utils.interpret', fromlist=['abx_summary']).abx_summary(extras["abx"])
        if abx_lines:
    ...
    meds_lines = __import__('utils.interpret', fromlist=['summarize_meds']).summarize_meds(meds) if meds else []
    abx_lines = __import__('utils.interpret', fromlist=['abx_summary']).abx_summary(extras.get("abx", {})) if extras.get("abx") else []
    cmp_lines = __import__('utils.interpret', fromlist=['compare_with_previous']).compare_with_previous(nickname, {k: vals.get(k) for k in ORDER if __import__('utils.inputs', fromlist=['entered']).entered(vals.get(k))}) if (mode=="일반/암") else []
    food_lines = __import__('utils.interpret', fromlist=['food_suggestions']).food_suggestions(vals, anc_place) if (mode=="일반/암") else []

    report_md = __import__('utils.reports', fromlist=['build_report']).build_report(mode, meta, vals, cmp_lines, extra_vals, meds_lines, food_lines, abx_lines)
    ...
    try:
        pdf_bytes = __import__('utils.reports', fromlist=['md_to_pdf_bytes_fontlocked']).md_to_pdf_bytes_fontlocked(report_md)
    ...
    if nickname and nickname.strip():
        rec = {
            ...
            "cancer": cancer,
            "infect": infect_sel,
            "labs": {k: vals.get(k) for k in ORDER if __import__('utils.inputs', fromlist=['entered']).entered(vals.get(k))},
            "extra": {k: v for k, v in (locals().get('extra_vals') or {}).items() if __import__('utils.inputs', fromlist=['entered']).entered(v)},
            "meds": meds,
            "extras": extras,
        }
...
# ===== Graphs =====
from utils.graphs import render_graphs as _render_graphs
_render_graphs()
