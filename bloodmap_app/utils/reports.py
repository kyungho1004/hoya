    ...
    if mode == "일반/암":
        ...
        for k in ORDER:
            ...
            try:
                ...
                continue
            if k == LBL_CRP: buf.append(f"- {k}: {fv:.2f}\n")
            else:
                if fv.is_integer(): buf.append(f"- {k}: {int(fv)}\n")
                else: buf.append(f"- {k}: {fv:.1f}\n")

        if compare_lines:
            buf.append("\n## 수치 변화 비교(이전 대비)\n")
            for l in compare_lines: buf.append(l + "\n")
        ...
        if meds_lines:
            buf.append("\n## 항암제 요약\n")
            for l in meds_lines: buf.append(l + "\n")

        if food_lines:
            buf.append("\n## 음식 가이드(계절/레시피 포함)\n")
            for l in food_lines: buf.append(l + "\n")

    if abx_lines:
        buf.append("\n## 항생제\n")
        for l in abx_lines: buf.append(l + "\n")
...
def md_to_pdf_bytes_fontlocked(md_text: str) -> bytes:
    ...
    if os.path.exists(FONT_PATH_B):
        try: pdfmetrics.registerFont(TTFont("NanumGothic-Bold", FONT_PATH_B))
        except Exception: pass
    if os.path.exists(FONT_PATH_XB):
        try: pdfmetrics.registerFont(TTFont("NanumGothic-ExtraBold", FONT_PATH_XB))
        except Exception: pass
    ...
    for line in md_text.splitlines():
        line = line.rstrip("\n")
        if not line.strip():
            story.append(Spacer(1, 4*mm)); continue
        if line.startswith("# "): p = Paragraph(f"<b>{escape(line[2:])}</b>", styles['Title'])
        elif line.startswith("## "): p = Paragraph(f"<b>{escape(line[3:])}</b>", styles['Heading2'])
