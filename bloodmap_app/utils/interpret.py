...
def interpret_labs(vals, extras):
    ...
    if _low(Hb, 8): out.append("🩸 Hb 낮음 — 빈혈 증상 모니터링, 어지럼/흉통 시 진료.")
...
def compare_with_previous(nickname, current_dict):
    # Simple placeholder: compare with last stored
...
def summarize_meds(meds: dict):
    ...
    for k, info in meds.items():
        ...
        if k.upper() in ["MTX","METHOTREXATE","메토트렉세이트"]:
            out.append(base + " — 간독성/구내염/골수억제/신장독성 주의, 고용량 시 류코보린 구제 고려.")
        elif k.upper() in ["ATRA","Vesanoid","Tretinoin".upper()]:
            out.append(base + " — 분화증후군(DS)·두통·피부건조 주의, 호흡곤란/부종 시 즉시 연락.")
        elif k.upper() in ["ARA-C","CYTARABINE"]:
            out.append(base + " — 고용량(HDAC) 시 소뇌실조, 결막염/각막염 가능.")
        elif k.upper() in ["G-CSF","FILGRASTIM"]:
...
_DEFAULT_ABX_TIPS = {
    ...
    "Cephalosporin": ["과민반응 병력 확인", "알코올과 상호작용(일부)"],
}
