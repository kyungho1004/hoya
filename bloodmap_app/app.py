...
def main():
    ...
    pin = st.text_input("PIN 4자리(중복 방지)", max_chars=4, help="별명 중복을 막기 위한 숫자 4자리", placeholder="예: 1024")
    if pin and (not pin.isdigit() or len(pin) != 4):
        st.warning("PIN은 숫자 4자리로 입력하세요. (예: 1024)")
    ...
    def ped_risk_banner(age_m, temp_c, rr, spo2, urine_24h, retraction, nasal_flaring, apnea):
        ...
        if urine_24h and urine_24h < 6: urgent=True; notes.append("소변 감소")
    ...
    # ===== Schedule =====
    nick_key = _nickname_key(nickname or "", pin or "")
    render_schedule(nick_key)
    ...
    if run:
        ...
        if mode == "일반/암":
            ...
            if nick_key and "records" in st.session_state and st.session_state.records.get(nick_key):
                st.markdown("### 🔍 수치 변화 비교 (이전 기록 대비)")
                cmp_lines = compare_with_previous(nick_key, {k: vals.get(k) for k in ORDER if entered(vals.get(k))})
                if cmp_lines:
        ...
        cmp_lines = compare_with_previous(nick_key, {k: vals.get(k) for k in ORDER if entered(vals.get(k))}) if (mode=="일반/암") else []
        ...
        try:
            ...
            st.info("PDF 생성 시 사용 폰트: NanumGothic(없으면 기본 폰트)")
        ...
        if nickname and nickname.strip():
            rec = {
                ...
                "extra": {k: v for k, v in (locals().get('extra_vals') or {}).items() if v is not None},
                "meds": meds,
                "extras": extras,
            }
            st.session_state.records.setdefault(nick_key, []).append(rec)
