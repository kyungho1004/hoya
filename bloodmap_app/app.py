...
def main():
    ...
    # local modules
    from config import (
        APP_TITLE, PAGE_TITLE, MADE_BY, CAFE_LINK_MD, FOOTER_CAFE,
    ...
    # ---- compatibility for utils (modular vs legacy) ----
    try:
        from bloodmap_app.utils.inputs import num_input_generic, entered, _parse_numeric   # type: ignore
        from bloodmap_app.utils.interpret import interpret_labs, compare_with_previous, food_suggestions, summarize_meds, abx_summary  # type: ignore
        from bloodmap_app.utils.reports import build_report, md_to_pdf_bytes_fontlocked    # type: ignore
        from bloodmap_app.utils.graphs import render_graphs                                 # type: ignore
        from bloodmap_app.utils.schedule import render_schedule                             # type: ignore
        from bloodmap_app.utils import counter as _bm_counter                               # type: ignore
    except Exception:
        from bloodmap_app.utils import (
            num_input_generic, entered, _parse_numeric,
            interpret_labs, compare_with_previous, food_suggestions, summarize_meds, abx_summary,
            build_report, md_to_pdf_bytes_fontlocked,
            render_graphs, render_schedule, counter as _bm_counter
        )
    ...
    def ped_risk_banner(age_m, temp_c, rr, spo2, urine_24h, retraction, nasal_flaring, apnea):
        ...
        if urine_24h and urine_24h < 3: urgent=True; notes.append("소변 감소(탈수 의심)")
    ...
    if run:
        ...
        try:
            ...
            st.info("PDF 생성 시 사용 폰트: NanumGothic(대체: Helvetica)")
            st.download_button(
                "🖨️ 보고서(.pdf) 다운로드",
                data=pdf_bytes,
            ...
            st.info("PDF 모듈/폰트가 없으면 .md/.txt로 사용하세요.")
    ...
    st.markdown("---")
    st.header("📚 약물 사전 (미니 뷰어)")
    with st.expander("열기 / 닫기", expanded=False):
        st.caption("빠르게 찾아보고 싶은 약을 검색하세요.")
        ...
        with view_tab1:
            ...
            for k, v in ANTICANCER.items():
                if q and (q.lower() not in k.lower()) and (q.lower() not in ",".join(v.get("aes", [])).lower()) and (q.lower() not in v.get("alias","").lower()):
                    continue
                with st.container(border=True):
                    st.markdown(f"**{k}** · {v.get('alias','')}")
                    st.write("부작용: " + ", ".join(v.get("aes", [])))
        with view_tab2:
            q2 = st.text_input("🔎 계열/주의사항 검색", key="drug_search_abx", placeholder="예: QT, 광과민, 와파린 ...")
            for cat, tips in ABX_GUIDE.items():
                if q2 and (q2.lower() not in cat.lower()) and (q2.lower() not in ",".join(tips).lower()):
                    continue
                with st.container(border=True):
                    st.markdown(f"**{cat}**")
                    st.write("주의사항: " + ", ".join(tips))
